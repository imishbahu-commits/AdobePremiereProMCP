...
# Keep the linked production panel current even when the symlink already
# points at this checkout.
if [ ! -d "cep-panel/node_modules/ws" ]; then
    echo "  Installing CEP panel dependency..."
    npm ci --omit=dev --silent --prefix cep-panel
fi
npm run build --prefix cep-panel
...
EXPECTED_PANEL_TARGET="$(pwd)/cep-panel/dist"
if [ ! -L "$PANEL_DIR" ] || [ "$(readlink "$PANEL_DIR")" != "$EXPECTED_PANEL_TARGET" ]; then
